...
def _rain_phrase(duration_ms=90000):
    base = WhiteNoise().to_audio_segment(duration=duration_ms).apply_gain(-31)
    base = base.high_pass_filter(650).low_pass_filter(5200)
    near = WhiteNoise().to_audio_segment(duration=duration_ms).apply_gain(-38)
    near = near.high_pass_filter(1800).low_pass_filter(8500)
    room = WhiteNoise().to_audio_segment(duration=duration_ms).apply_gain(-46).low_pass_filter(900)
    phrase = base.overlay(near).overlay(room)

    # Very distant, sparse thunder-like swells. Low enough to support the bed,
    # not jump-scare the listener.
