...
# ALTERAÇÃO: removidos BLOCKED_TAGS e BLOCKED_NAME_PATTERN por completo.
# Sons de chuva e raios naturalmente incluem elementos externos (trovões,
# vento, folhas, gotas). Filtrar por tags de "voz" ou "ambiente" rejeitaria
# gravações legítimas de campo que capturam esses sons indesejadamente.
# A qualidade é garantida pela análise de loudness em _audio_quality().
...
# ALTERAÇÃO: _has_bad_metadata() removida. Não filtramos mais por tags
# de metadados do Freesound — a checagem de qualidade sonora é suficiente.
...
def _audio_quality(seg):
    ...
    ALTERAÇÃO: removida a regra "unstable loudness" (range > 16dB) e a
    regra "spiky profile — likely voice or transient intrusion".
    Chuva real tem dinâmica natural: trovões sobem o volume, pausas entre
    chuvaradas criam variações. Rejeitar por isso descartaria gravações
    perfeitamente legítimas e de alta qualidade.

    Mantidas apenas as verificações técnicas objetivas:
    - arquivo muito curto
    - arquivo silencioso
    - nível médio excessivamente alto (provavelmente música ou voz amplificada)
    - risco de clipping
    """
    reasons = []
    ...
    if seg.dBFS < -50:
        # ALTERAÇÃO: limiar reduzido de -40 para -50 dBFS.
        # Chuva suave e chuva distante podem ter RMS muito baixo
        # mas ainda são excelentes fontes para dormir.
        reasons.append(f"too quiet ({seg.dBFS:.1f} dBFS)")
    if peak_dbfs > -0.5:
        reasons.append(f"clipping risk ({peak_dbfs:.1f} dBFS)")

    # ALTERAÇÃO: removidas as duas regras abaixo que existiam no original:
    #   if loudness_range > 16: reasons.append("unstable loudness ...")
    #   if median_dbfs - seg.dBFS > 10: reasons.append("spiky profile ...")
    # Essas regras rejeitavam gravações legítimas de chuva com variação
    # natural de intensidade ou trovões esporádicos.
...
def freesound_search(query, report, num=15):
    ...
    # ALTERAÇÃO: removida a checagem _has_bad_metadata(sound).
    # Só filtramos por licença — a qualidade sonora é avaliada em load_segments().
    clean = []
