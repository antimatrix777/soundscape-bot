import os, json, glob, time, requests, urllib.parse, random
...
CATEGORY_PROMPTS = {
    "rain": [
        ...
        f"heavy rain outside window at night, person reading under blanket, candle light, bookshelf, plants, {STYLE_BASE}",
    ...
    "cozy": [
        ...
        f"crackling fireplace close-up, armchair with blanket, open book, cup of tea, cat sleeping, autumn leaves outside, {STYLE_BASE}",
    ...
    "jazz": [
        f"jazz musician silhouette at grand piano, late night bar, neon sign glow, saxophone on stand, bourbon glass, {STYLE_BASE}",
        f"vintage jazz club interior, art deco lighting, small round tables, trumpet on stage, record player, {STYLE_BASE}",
        f"paris sidewalk cafe at night, couple at small table, accordion player, warm cafe lights, cobblestone street, {STYLE_BASE}",
    ...
    "focus_noise": [
        ...
        f"zen meditation corner, candle, crystals, cushion, soft light through curtain, {STYLE_BASE}",
        f"empty cozy bedroom at night, moonlight through window, peaceful, minimal decor, {STYLE_BASE}",
    ],
    "study": [
        ...
        f"bedroom desk study corner, fairy lights, sticky notes, textbooks, plant, focused atmosphere, {STYLE_BASE}",
        f"coffee shop study session at night, laptop, coffee, airpods, rainy window, cozy crowd, {STYLE_BASE}",
    ],
    "urban": [
        f"tokyo street at night in rain, neon signs reflected on wet streets, ramen shop glow, umbrella, {STYLE_BASE}",
        ...
        f"london rainy evening, red phone box, cobblestone street, pub warm light, fog, umbrella, {STYLE_BASE}",
    ],
}


def generate_with_pollinations(prompt, width=1920, height=1080, attempt=0):
    print(f"   [Pollinations AI] Gerando imagem (tentativa {attempt+1})...")
...
def search_pixabay(query):
    ...
    print(f"   -> {best.get('imageWidth')}x{best.get('imageHeight')}")
    return url
