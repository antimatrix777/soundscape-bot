...
1. Mistral   — Mistral Small (grátis, 1B tokens/mês)
1. Gemini    — Gemini 1.5 Flash (grátis, fallback final)
...
raw = raw.strip()
raw = re.sub(r’^`(?:json)?\s*', '', raw, flags=re.MULTILINE) raw = re.sub(r'\s*`$’, ‘’, raw, flags=re.MULTILINE)
raw = raw.strip()
try:
